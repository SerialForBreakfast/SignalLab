...
Replace browser-specific modes with **catalog / crash / exception / breakpoint / retain / hang / cpu / thread / zombie / tsan / malloc / heap / deadlock** in `grab_screenshot.sh`. Prefer **timestamped files** in `memlog/ui-review/` so prior captures are preserved (same spirit as JoesProxy’s `joesproxy-screenshot-*.png` naming).
