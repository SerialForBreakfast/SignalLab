...
enum LabCatalog {
    ...
    private static let memoryGraphLab = LabScenario(
        ...
        summary: "Create one checkout session, keep it alive in a long-lived session store, and use Xcode Memory Graph to see the owner arrow and allocation backtrace.",
        category: .memory,
        difficulty: .beginner,
        learningGoals: [
        ...
        reproductionSteps: [
            ...
            "Tap Set up lab. The app creates a checkout session and stores it in MemoryGraphSessionStore.",
            ...
            "Select MemoryGraphSessionStore.",
            "Follow the currentSession arrow to MemoryGraphCheckoutSession. Read that arrow as: the store keeps this session alive.",
            "Select MemoryGraphCheckoutSession, open the right inspector, and expand Backtrace.",
            "Select the MemoryGraphCheckoutSession.__allocating_init(identifier:) frame, then click the tiny arrow.up.forward.circle-style jump-to-source button at the far right of that row.",
            "Tap Reset, capture Memory Graph again, and confirm currentSession no longer points to the checkout session.",
        ],
        hints: [
        ...
        investigationGuide: InvestigationGuide(
            steps: [
                ...
                "Tap Set up lab to create the stored checkout session.",
                "Open Memory Graph and select MemoryGraphSessionStore under SignalLab.debug.dylib.",
                "Follow currentSession to MemoryGraphCheckoutSession.",
                "Use the right inspector Backtrace. Select the MemoryGraphCheckoutSession.__allocating_init(identifier:) frame, then click the tiny jump-to-source arrow at the far right of that row.",
                "Tap Reset, capture Memory Graph again, and confirm the store no longer keeps the session alive.",
            ],
            validationChecklist: [
